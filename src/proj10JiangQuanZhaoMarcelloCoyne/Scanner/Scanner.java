...
package proj10JiangQuanZhaoMarcelloCoyne.Scanner;

import proj10JiangQuanZhaoMarcelloCoyne.Scanner.Token.Kind;
...
public class Scanner {
    ...
    private void registerError(Error.Kind kind, String message) {
        ...
        if (this.sourceFile.getCurrentChar() == SourceFile.eol) {
            position--;
        }
    ...
    private Token createNewToken(Token.Kind kind, String spelling, boolean ifGetNextChar) {
        ...
        if (ifGetNextChar) {
            ...
            this.currentChar = this.sourceFile.getCurrentChar();
        }
        return newToken;
    }
    ...
    private Token constructSpecialCharWithSameFirstLetterToken(char firstChar, char secondChar, Token.Kind kind_long, Token.Kind kind_short) {
        if (this.sourceFile.getNextChar() == secondChar) {
    ...
    private Token constructBlockCommentToken() {
        ...
        while (!(this.currentChar == '*' && this.sourceFile.getNextChar() == '/')) {
            ...
            if (this.sourceFile.getNextChar() == SourceFile.eof) {
    ...
    public Token scan() {
        try {
            ...
            // -------------------- Block Comment
            else if (this.currentChar == '/' && this.sourceFile.getCurrentChar() == '*') {
